(function(){
	...
	jesong.util={
				...
				getReseveKey:function(){
					...
					if(typeof(JESONG_EXT_DATA) != "undefined" && JESONG_EXT_DATA != null){
						return JESONG_EXT_DATA;
					}
			...
			openChat:function(c){
				...
				if(jesong.jsType == 1 && jesong.env.isPhone){
					...
					jesong.chatRequest = true;
				}else if(ev.min == 1 && jesong.force && jesong.force.openchat){
	...
	jesong.monitor.pump=function(){
		...
		switch(r.cmd){
			...
			case 'getEvent':
				...
				if(jesong.jsType == 1 && jesong.config.phoneChatPop==1 && jesong.env.isPhone && jesong.resp && jesong.resp.chatId && jesong.resp.customerId){
					if(jesong.force && jesong.force.status != 2 && jesong.chatRequest){
						jesong.chatRequest = false;
						jesong.force.connect(jesong.resp.chatId, jesong.resp.customerId);
					}
				}
				break;
	...
	jesong.monitor.accept=function(){
		...
		u.show('jesong_panel');
		if(jesong.jsType == 1 && jesong.config.phoneChatPop==1 && jesong.env.isPhone){
			jesong.chatRequest = true;
		}
	...
	jesong.monitor.show=function(){
		if((jesong.force && (jesong.force.status == 2 || (jesong.autochat && jesong.autochat.use == 1 && jesong.autochat.show))) || jesong.chatRequest){
			return;
		}
		var u = jesong.util;
		if(jesong.monitor.config.mask){
	...
	jesong.monitor.record=function(){
		...
		var url = ev.server.monitor+'i?cmd=add&c='+ev.compId+'&v='+ev.vId+'&u='+ev.uId+'&p='+ev.pId
			//+'device='+(jesong.env.isPhone?"phone":"pc")
	...
	jesong.monitor.init=function(){
		...
		if(mon.config.type == 1){
			...
			m.innerHTML='<div class="title" id="jesong_mon_head">'+mon.config.title+'</div><div class="context">'+mon.config.text+'</div><div class="accept" onclick="javascript:jesong.monitor.accept()"></div><div class="refuse" onclick="jesong.monitor.refuse()"></div>';
	...
	jesong.panel.init=function(){
		...
		if(con.position != -1){
			var cssText = "position:fixed;_position:absolute;"
					+((con.position==0)?'left':'right')+":"+con.horizon+"px;"
					+"top:"+con.vertical+"px;"
					+"_buttom:auto;_top:expression(eval(document.documentElement.scrollTop+"+con.vertical+"));";
	...
	jesong.icon.builder=function(){
			...
			var html = "<div style=\"width:100%;height:100%;background:url("+ (con.status==1?con.online:con.offline) + ") no-repeat;\"></div>";
	...
	jesong.util.winChat = function(commonFun){
		...
		if(params && params.chatting){
			if(jesong.env.min != 1 || (jesong.jsType == 1 && jesong.env.isPhone)){
	...
	jesong.loadForceJS = function(){
		jesong.util.loadJS(jesong.env.server.file+"js/force.js?ver="+jesong.version);
	},
